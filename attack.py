...
def bobs_custom_pwhash_attack(pc, char_list, length, pwlist_obj):
    ...
    def parts_hash(pwhash):
        pwd_length = len(pwhash)
        pieceSize = pwd_length // 3
        return pwhash[:pieceSize], pwhash[pieceSize:2 * pieceSize], pwhash[2 * pieceSize:]

    def pwHasher(p, salt):
        return crypto.truncated_sha2_hex_digest(p + '-' + salt, 6)

    salts, hashedPassword , part1, part2 = [], [], [], []
    ...
    for pwitem in pwlist_obj:
        ...
        p1, p2, salt_c = parts_hash(pwhash)
        part1.append(p1)
        part2.append(p2)
        salts.append(salt)
        hashedPassword.append(pwhash)

    pwGenerator1 = crypto.candidate_bf_generator(char_list, length // 2)

    pwGenerator2 = crypto.candidate_bf_generator(char_list, length // 2)

    async def attack_part(pw_gen, hash_part, pwHasher, salt):
        return await asyncio.to_thread(simple_exhaustive_attack, pc, pw_gen, lambda p: pwHasher(p, salt), hash_part)

    async def main():
        tasks = [
            attack_part(pwGenerator1, part1, pwHasher, 'p1'),
            attack_part(pwGenerator2, part2, pwHasher, 'p2')
        ]
        completed, _ = await asyncio.wait(tasks)
        results[:2] = [task.result() for task in completed]
    ...
    for i in range(len(pwlist_obj)):
        password, salt, pwhash = results[i], salts[i], hashedPassword[i]
        # assert pwHasher(password, salt) == pwhash
